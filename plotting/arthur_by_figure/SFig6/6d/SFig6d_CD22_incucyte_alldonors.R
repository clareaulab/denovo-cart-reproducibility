...
k562_df <- cd22_incucyte_summary %>% filter(cellline == "K562")
...
k562_cd22_df = cd22_incucyte_summary %>% filter(cellline == "K562 CD22")
...
rpmi_df = cd22_incucyte_summary %>% filter(cellline == "RPMI-8226")

rpmi_plot = ggplot(rpmi_df, aes(x = elapsed_hours, y = mean_conf,
                                color = condition, fill = condition)) +
  geom_errorbar(aes(ymin = mean_conf - sem_conf,
                    ymax = mean_conf + sem_conf),
                width = 1, linewidth= 0.3, alpha = 1) +
  geom_line(linewidth =0.8) +
  geom_point(size = 1.5) +
  scale_color_manual(values = cond_colors) +
  scale_fill_manual(values  = cond_colors) +
  ...
  labs(title = "RPMI-8226")

rpmi_plot
ggsave2(rpmi_plot,
        filename = "../plots/DNCT_rev/CD22_incucyte_RPMI-8226_alldonors.pdf",
        width = 2.5, height = 2)
